...
class DEBLUR(object):
    ...
    def input_producer(self, batch_size=10):
        # img_a is sharp image, img_b is corresponding blur image
        def read_data():
        ...
        # tf.stack(imgs, axis = 0) 是直接把img在行上叠加， 原来image 是 height * width * 3, stack 之后就是 2 * height * width * 3
        # tf.random_crop 第一参数是图像源，第二个参数是切割大小, 也就是把两张图片随机裁剪成 256 * 256
        # tf.unstack 然后再把stack的逆操作恢复成两张图片
        def preprocessing(imgs):
            imgs = [tf.cast(img, tf.float32) / 255.0 for img in imgs]  #tf.case 数据格式的转换
            if self.args.model != 'color':
                imgs = [tf.image.rgb_to_grayscale(img) for img in imgs] #转成灰度
            img_crop = tf.unstack(tf.random_crop(tf.stack(imgs, axis=0), [2, self.crop_size, self.crop_size, self.chns]),
                                  axis=0) # tf.stack会在新的张量阶上拼接，产生的张量的阶数将会增加
            #random_crop随机切割
            return img_crop

        with tf.variable_scope('input'):
            List_all = tf.convert_to_tensor(self.data_list, dtype=tf.string)
            gt_list = List_all[:, 0] #gound truth
            in_list = List_all[:, 1] # blur
        ...
        with tf.variable_scope(scope, reuse=reuse):
            with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
                ...
                for i in xrange(self.n_levels):
                    ...
                    # 还cat了一半 来自上一个scale的数据！
                    # 对于这一半数据，阻止梯度的传递
                    # 在index 为3的维度上cat，所以是两张resize到相同size的三通道图片cat，得到的张量是 [batch_size, h ,w , 6]
    ...
    def build_model(self):
        ...
        for i in xrange(self.n_levels):
            ...
            gradient_gt_i_x, gradient_gt_i_y = tf.image.image_gradients(gt_i)
            gradient_pred_x, gradient_pred_y = tf.image.image_gradients(x_unwrap[i])
            ...
            loss_ssim = 1 - tf.reduce_mean((ssim_x **2 + ssim_y ** 2)/2)
            self.loss_total += (loss + loss_ssim)
    ...
    def train(self):
        ...
        # learning rate decay ， learning rate 的衰减方法
        ...
        # self.load(sess, self.train_dir, step=69000)
    ...
    def test(self, height, width, input_path, output_path):
        ...
        for imgName in imgsName:
            ...
            blurPad = np.expand_dims(blurPad, 0) #增加维度，因为batch_size = 1
            if self.args.model != 'color':
            ...
            res = deblur[-1] # 因为有三个scale，最后一个是原大小的
            print(res.shape)
            if self.args.model != 'color':
                res = np.transpose(res, (3, 1, 2, 0))
            res = im2uint8(res[0, :, :, :]) #把像素值的动态范围恢复到 0 - 255
