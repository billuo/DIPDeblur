...
### Generator layers
def c7s1_k(input, origin, k, reuse=False, norm='instance', activation='relu', is_training=True, skip=False, name='c7s1_k'):
  ...
  with tf.variable_scope(name, reuse=reuse):
    ...
    if (skip==False) or (k!=3):
      ...
      if activation == 'relu':
        output = tf.nn.relu(normalized+origin)
      if activation == 'tanh':
        output = tf.nn.tanh(normalized+origin)
    return output
